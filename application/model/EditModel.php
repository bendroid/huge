...
class EditModel
{
    ...
    public static function editUserMobile($new_mobile_phone)
    {
        ...
        if ($new_mobile_phone == Session::get('mobile_phone')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_MOBILE_PHONE_SAME_AS_OLD_ONE'));
            return false;
        }
        ...
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_mobile_phone)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_MOBILE_PHONE_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/
     ...
     * @return bool success status
     */
    public static function editUserFirstName($new_first_name)
    {
        // new first name phone same as old one ?
        if ($new_first_name == Session::get('first_name')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_FIRST_NAME_SAME_AS_OLD_ONE'));
            return false;
        }

		/*
        // first name phone cannot be empty and must be azAZ09 and 2-64 characters
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_first_name)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_FIRST_NAME_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/
        ...
        if ($status_of_action) {
            ...
            Session::add('feedback_positive', Text::get('FEEDBACK_FIRST_NAME_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
...
/**
     * Writes new first name phone address to database
     *
     * @param $user_id int user id
     * @param $new_first_name string new first name phone address
     *
     * @return bool
     */
     ...
     * @return bool success status
     */
    public static function editUserLastName($new_last_name)
    {
        // new last name phone same as old one ?
        if ($new_last_name == Session::get('last_name')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_LAST_NAME_SAME_AS_OLD_ONE'));
            return false;
        }

		/*
        // last name phone cannot be empty and must be azAZ09 and 2-64 characters
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_last_name)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_LAST_NAME_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/
        ...
        if ($status_of_action) {
            ...
            Session::add('feedback_positive', Text::get('FEEDBACK_LAST_NAME_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
...
/**
     * Writes new last name phone address to database
     *
     * @param $user_id int user id
     * @param $new_last_name string new last name phone address
     *
     * @return bool
     */
	...
	/**
     * Edit the user's full name, provided in the editing form
     *
     * @param $new_full_name string The new username
     *
     * @return bool success status
     */
    public static function editUserFullName($new_full_name)
    {
        // new full name same as old one ?
        if ($new_full_name == Session::get('full_name')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_FULL_NAME_SAME_AS_OLD_ONE'));
            return false;
        }

		/*
        // last name phone cannot be empty and must be azAZ09 and 2-64 characters
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_full_name)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_FULL_NAME_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/

        // clean the input, strip phone numbers longer than 64 chars (maybe fix this ?)
        $new_full_name = substr(strip_tags($new_full_name), 0, 64);


        $status_of_action = self::saveNewFullName(Session::get('user_id'), $new_full_name);
        if ($status_of_action) {
            Session::set('full_name', $new_full_name);
            Session::add('feedback_positive', Text::get('FEEDBACK_FULL_NAME_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
...
/**
     ...
     *
     * @param $user_id int user id
     * @param $new_full_name string new last name phone address
     *
     * @return bool
     */
    public static function saveNewFullName($user_id, $new_full_name)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $query = $database->prepare("UPDATE users SET full_name = :full_name WHERE user_id = :user_id LIMIT 1");
        $query->execute(array(':full_name' => $new_full_name, ':user_id' => $user_id));
        $count = $query->rowCount();
        if ($count == 1) {
            return true;
        }
    ...
    public static function editUserHome($new_home_phone)
    {
        ...
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_home_phone)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_HOME_PHONE_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/
        ...
        if ($status_of_action) {
            ...
            Session::add('feedback_positive', Text::get('FEEDBACK_HOME_PHONE_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
    ...
    public static function editUserWork($new_work_phone)
    {
        ...
        if ($new_work_phone == Session::get('work_phone')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_WORK_PHONE_SAME_AS_OLD_ONE'));
            return false;
        }
        ...
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_work_phone)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_WORK_PHONE_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/
        ...
        if ($status_of_action) {
            ...
            Session::add('feedback_positive', Text::get('FEEDBACK_WORK_PHONE_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
    ...
    public static function editUserFB($new_user_fb)
    {
        ...
        if ($new_user_fb == Session::get('user_fb')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_FACEBOOK_SAME_AS_OLD_ONE'));
            return false;
        }
        ...
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_user_fb)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_FACEBOOK_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/
        ...
        if ($status_of_action) {
            ...
            Session::add('feedback_positive', Text::get('FEEDBACK_FACEBOOK_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
    ...
    public static function editUserGH($new_user_github)
    {
        ...
        if ($new_user_github == Session::get('user_github')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_GITHUB_SAME_AS_OLD_ONE'));
            return false;
        }
        ...
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_user_github)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_GITHUB_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/
        ...
        if ($status_of_action) {
            ...
            Session::add('feedback_positive', Text::get('FEEDBACK_GITHUB_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
    ...
    public static function editUserTW($new_user_twitter)
    {
        ...
        if ($new_user_twitter == Session::get('user_twitter')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_TWITTER_SAME_AS_OLD_ONE'));
            return false;
        }
        ...
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_user_twitter)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_TWITTER_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/
        ...
        if ($status_of_action) {
            ...
            Session::add('feedback_positive', Text::get('FEEDBACK_TWITTER_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
     ...
     * Edit the user's full name, provided in the editing form
     *
     * @param $new_user_gp string The new username
     *
     * @return bool success status
     */
    public static function editUserGP($new_user_gp)
    {
        // new last name phone same as old one ?
        if ($new_user_gp == Session::get('user_gp')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_GOOGLE_PLUS_SAME_AS_OLD_ONE'));
            return false;
        }

		/*
        // last name phone cannot be empty and must be azAZ09 and 2-64 characters
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_user_gp)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_GOOGLE_PLUS_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/

        // clean the input, strip phone numbers longer than 64 chars (maybe fix this ?)
        $new_user_gp = substr(strip_tags($new_user_gp), 0, 64);


        $status_of_action = self::saveNewUserGP(Session::get('user_id'), $new_user_gp);
        if ($status_of_action) {
            Session::set('user_gp', $new_user_gp);
            Session::add('feedback_positive', Text::get('FEEDBACK_GOOGLE_PLUS_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
...
/**
     ...
     *
     * @param $user_id int user id
     * @param $new_user_gp string new last name phone address
     *
     * @return bool
     */
    public static function saveNewUserGP($user_id, $new_user_gp)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $query = $database->prepare("UPDATE users SET user_gp = :user_gp WHERE user_id = :user_id LIMIT 1");
        $query->execute(array(':user_gp' => $new_user_gp, ':user_id' => $user_id));
        $count = $query->rowCount();
        if ($count == 1) {
            return true;
        }
     ...
     * Edit the user's full name, provided in the editing form
     *
     * @param $new_user_street string The new username
     *
     * @return bool success status
     */
    public static function editUserStreet($new_user_street)
    {
        // new last name phone same as old one ?
        if ($new_user_street == Session::get('user_street')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_STREET_SAME_AS_OLD_ONE'));
            return false;
        }

		/*
        // last name phone cannot be empty and must be azAZ09 and 2-64 characters
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_user_street)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_STREET_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/

        // clean the input, strip phone numbers longer than 64 chars (maybe fix this ?)
        $new_user_street = substr(strip_tags($new_user_street), 0, 64);


        $status_of_action = self::saveNewUserStreet(Session::get('user_id'), $new_user_street);
        if ($status_of_action) {
            Session::set('user_street', $new_user_street);
            Session::add('feedback_positive', Text::get('FEEDBACK_STREET_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
     ...
     *
     * @param $user_id int user id
     * @param $new_user_street string new last name phone address
     *
     * @return bool
     */
    public static function saveNewUserStreet($user_id, $new_user_street)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $query = $database->prepare("UPDATE users SET user_street = :user_street WHERE user_id = :user_id LIMIT 1");
        $query->execute(array(':user_street' => $new_user_street, ':user_id' => $user_id));
        $count = $query->rowCount();
        if ($count == 1) {
            return true;
        }
     ...
     * Edit the user's full name, provided in the editing form
     *
     * @param $new_user_city string The new username
     *
     * @return bool success status
     */
    public static function editUserCity($new_user_city)
    {
        // new last name phone same as old one ?
        if ($new_user_city == Session::get('user_city')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_CITY_SAME_AS_OLD_ONE'));
            return false;
        }

		/*
        // last name phone cannot be empty and must be azAZ09 and 2-64 characters
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_user_city)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_CITY_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/

        // clean the input, strip phone numbers longer than 64 chars (maybe fix this ?)
        $new_user_city = substr(strip_tags($new_user_city), 0, 64);


        $status_of_action = self::saveNewUserCity(Session::get('user_id'), $new_user_city);
        if ($status_of_action) {
            Session::set('user_city', $new_user_city);
            Session::add('feedback_positive', Text::get('FEEDBACK_CITY_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
...
/**
     ...
     *
     * @param $user_id int user id
     * @param $new_user_city string new last name phone address
     *
     * @return bool
     */
    public static function saveNewUserCity($user_id, $new_user_city)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $query = $database->prepare("UPDATE users SET user_city = :user_city WHERE user_id = :user_id LIMIT 1");
        $query->execute(array(':user_city' => $new_user_city, ':user_id' => $user_id));
        $count = $query->rowCount();
        if ($count == 1) {
            return true;
        }
     ...
     * Edit the user's full name, provided in the editing form
     *
     * @param $new_user_state string The new username
     *
     * @return bool success status
     */
    public static function editUserState($new_user_state)
    {
        // new last name phone same as old one ?
        if ($new_user_state == Session::get('user_state')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_STATE_SAME_AS_OLD_ONE'));
            return false;
        }

		/*
        // last name phone cannot be empty and must be azAZ09 and 2-64 characters
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_user_state)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_STATE_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/

        // clean the input, strip phone numbers longer than 64 chars (maybe fix this ?)
        $new_user_state = substr(strip_tags($new_user_state), 0, 64);


        $status_of_action = self::saveNewUserState(Session::get('user_id'), $new_user_state);
        if ($status_of_action) {
            Session::set('user_state', $new_user_state);
            Session::add('feedback_positive', Text::get('FEEDBACK_STATE_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
...
/**
     ...
     *
     * @param $user_id int user id
     * @param $new_user_state string new last name phone address
     *
     * @return bool
     */
    public static function saveNewUserState($user_id, $new_user_state)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $query = $database->prepare("UPDATE users SET user_state = :user_state WHERE user_id = :user_id LIMIT 1");
        $query->execute(array(':user_state' => $new_user_state, ':user_id' => $user_id));
        $count = $query->rowCount();
        if ($count == 1) {
            return true;
        }
     ...
     * Edit the user's full name, provided in the editing form
     *
     * @param $new_user_zip string The new username
     *
     * @return bool success status
     */
    public static function editUserZip($new_user_zip)
    {
        // new last name phone same as old one ?
        if ($new_user_zip == Session::get('user_zip')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_ZIP_SAME_AS_OLD_ONE'));
            return false;
        }

		/*
        // last name phone cannot be empty and must be azAZ09 and 2-64 characters
        if (!preg_match("/^[a-zA-Z0-9]{2,64}$/", $new_user_zip)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_ZIP_DOES_NOT_FIT_PATTERN'));
            return false;
        }
		*/

        // clean the input, strip phone numbers longer than 64 chars (maybe fix this ?)
        $new_user_zip = substr(strip_tags($new_user_zip), 0, 64);


        $status_of_action = self::saveNewUserZip(Session::get('user_id'), $new_user_zip);
        if ($status_of_action) {
            Session::set('user_zip', $new_user_zip);
            Session::add('feedback_positive', Text::get('FEEDBACK_ZIP_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_UNKNOWN_ERROR'));
            return false;
...
/**
     ...
     *
     * @param $user_id int user id
     * @param $new_user_zip string new last name phone address
     *
     * @return bool
     */
    public static function saveNewUserZip($user_id, $new_user_zip)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $query = $database->prepare("UPDATE users SET user_zip = :user_zip WHERE user_id = :user_id LIMIT 1");
        $query->execute(array(':user_zip' => $new_user_zip, ':user_id' => $user_id));
        $count = $query->rowCount();
        if ($count == 1) {
            return true;
        }
